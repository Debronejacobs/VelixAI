    ...
    try {
      ...
      if (error) {
        if ((error as any).status === 429) {
          ...
          setErrorMessage('Error connecting to system.');
        }
        throw error;
      }
  ...
  return (
    <section className="relative w-full h-[80vh] flex flex-col items-center justify-center bg-transparent overflow-hidden z-10">
      ...
      >
        ...
        {status === 'error' && (
          <p className="text-red-500 font-mono text-sm mt-4 tracking-widest uppercase">
            {errorMessage || 'ERROR_INITIALIZING_CONNECTION'}
          </p>
        )}
      </motion.div>
    </section>
